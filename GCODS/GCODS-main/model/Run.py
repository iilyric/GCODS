...
def run_training_job(trial=None):
    ...
    args.add_argument('--lr_scheduler_type', default=config['train'].get('lr_scheduler_type', 'cosine'), type=str)
    args.add_argument('--lr_min', default=config['train'].getfloat('lr_min', 1e-6), type=float)
    ...
    args.add_argument('--use_mcdp_net', default=config['model'].getboolean('use_mcdp_net', False), type=eval)
    ...
    if config.has_section('mcdp_params'):
        args.add_argument('--gcn_depth', default=config['mcdp_params'].getint('gcn_depth', 2), type=int)
        args.add_argument('--mcdp_dropout', default=config['mcdp_params'].getfloat('dropout', 0.3), type=float)
        args.add_argument('--mcdp_alpha', default=config['mcdp_params'].getfloat('alpha', 0.05), type=float)
        args.add_argument('--mcdp_beta', default=config['mcdp_params'].getfloat('beta', 0.95), type=float)
        args.add_argument('--mcdp_gamma', default=config['mcdp_params'].getfloat('gamma', 0.95), type=float)
        args.add_argument('--node_dim', default=config['mcdp_params'].getint('node_dim', 40), type=int)
        args.add_argument('--hyperGNN_dim', default=config['mcdp_params'].getint('hyperGNN_dim', 16), type=int)
    else:
        args.add_argument('--gcn_depth', default=2, type=int)
        args.add_argument('--mcdp_dropout', default=0.3, type=float)
        args.add_argument('--mcdp_alpha', default=0.05, type=float)
        args.add_argument('--mcdp_beta', default=0.95, type=float)
        args.add_argument('--mcdp_gamma', default=0.95, type=float)
    ...
    if parsed_args.lr_decay:
        sch_type = parsed_args.lr_scheduler_type.lower()
        if sch_type == 'cosine':
            lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer=optimizer,
                T_max=parsed_args.epochs,
                eta_min=parsed_args.lr_min,
            )
            print(f'Applying cosine-annealing LR decay: '
                  f'lr_init={parsed_args.lr_init} -> eta_min={parsed_args.lr_min} '
                  f'over {parsed_args.epochs} epochs.')
            ...
            print(f'Applying multi-step LR decay: '
                  f'milestones={lr_decay_steps}, gamma={parsed_args.lr_decay_rate}.')
        else:
            raise ValueError(
                f"Unknown lr_scheduler_type: '{sch_type}'. "
                f"Valid options: 'cosine', 'multistep'."
            )
...
if __name__ == '__main__':
    ...
    args_main_parser.add_argument('--lr_scheduler_type', default=config_main['train'].get('lr_scheduler_type', 'cosine'), type=str)
    args_main_parser.add_argument('--lr_min', default=config_main['train'].getfloat('lr_min', 1e-6), type=float)
    ...
    args_main_parser.add_argument('--use_mcdp_net', default=config_main['model'].getboolean('use_mcdp_net', False), type=eval)
    if config_main.has_section('mcdp_params'):
        args_main_parser.add_argument('--gcn_depth', default=config_main['mcdp_params'].getint('gcn_depth', 2), type=int)
        args_main_parser.add_argument('--mcdp_dropout', default=config_main['mcdp_params'].getfloat('dropout', 0.3), type=float)
        args_main_parser.add_argument('--mcdp_alpha', default=config_main['mcdp_params'].getfloat('alpha', 0.05), type=float)
        args_main_parser.add_argument('--mcdp_beta', default=config_main['mcdp_params'].getfloat('beta', 0.95), type=float)
        args_main_parser.add_argument('--mcdp_gamma', default=config_main['mcdp_params'].getfloat('gamma', 0.95), type=float)
        args_main_parser.add_argument('--node_dim', default=config_main['mcdp_params'].getint('node_dim', 40), type=int)
        args_main_parser.add_argument('--hyperGNN_dim', default=config_main['mcdp_params'].getint('hyperGNN_dim', 16), type=int)
